...
只为上下文隔离或可测并行收益引入子 Agent。

- 前置：[f04-deep-agents](../f04-deep-agents/)
- 产物：`architecture comparison`
- 能力：`subagent-isolation`, `parallel-work`

## 1. Frame

先回答：**并行收益是否超过协调成本和错误面？**
...
```text
delegate → isolate → join
```

## 3. Build

完成或审查 `architecture comparison`。实现入口：
...
关联 suite：`frontier-subagents-candidate`
